...
class BranchAndPrice:
    ...
    def solve(self):
        ...
        while not self.branch_queue.empty():
            ...
            log_text = (f"LP iters: {lp_iters}, solved node id: {node_id}, queue size: {self.branch_queue.qsize()}, "
    ...
    def branch(self, current_node_id, branch_candidates, var_vals: dict):
        global node_id_counter
        current_node = node_infos[current_node_id]
        # left node
        node_id_counter += 1
        ...
        if not is_integer(sum_vals):
            ...
            combined_binary = [(k, v, 'truck') for k, v in truck_arc_flows.items() if
                               0 < v < 1 and not is_integer(v)] + [
                                  (k, v, 'drone') for k, v in drone_arc_flows.items() if
                                  0 < v < 1 and not is_integer(v)]
            ...
            if len(combined_binary) > 0:
                ...
                combined_integer = [(k, v, 'truck') for k, v in truck_arc_flows.items() if
                                    v > 1 and not is_integer(v)] + [
                                       (k, v, 'drone') for k, v in drone_arc_flows.items() if
                                       v > 1 and not is_integer(v)]
                ...
                select_columns = arc_travel_column_keys[arc]
                node_left.arc_flow_ub_cons_dict[(node_i, node_j, flow)].add(select_columns)
                node_right.arc_flow_lb_cons_dict[(node_i, node_j, flow)].add(select_columns)
